  ...
  const listFilter = (listItems, listName) => {
    ...
    if (listName == "trabajadores") {
      const filtered = listItems.filter((item) =>
        item.trabajador.toLowerCase().includes(query)
      );
      return filtered;
    }
    if (listName == "aportes") {
      const filtered = listItems.filter((item) =>
        item.trabajador?.trabajador.toLowerCase().includes(query)
      );
      return filtered;
    }
    if (listName == "gastos") {
      const filtered = listItems.filter((item) =>
        item.tipo_gasto?.tipo_gasto.toLowerCase().includes(query)
      );
      return filtered;
    }
    ...
    if (listName == "ventas") {
      const filtered = listItems.filter(
        (item) =>
          item.cliente?.nombres.toLowerCase().includes(query) ||
          item.cliente?.apellidos.toLowerCase().includes(query)
      );
      return filtered;
    }
