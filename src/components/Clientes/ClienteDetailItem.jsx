...
const ClienteDetailItem = () => {
  ...
  const calcularPromedioDias = () => {
    const totalSuma = ventasActivas.reduce(
      (total, venta) => total + venta.dias_atrasados,
      0
    );
      ...
      {loading ? (
        ...
        <div className="card shadow-lg p-3 mb-5 bg-body rounded">
          ...
          <div className="card-footer d-flex flex-wrap justify-content-around">
            ...
            <Link to={"/clientes/update/"} className="btn btn-warning mb-1">
              Actualizar
            </Link>
            <Link to={"/clientes/delete/"} className="btn btn-danger mb-1">
              Eliminar
            </Link>
          </div>
        </div>
