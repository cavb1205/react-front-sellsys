...
const TrabajadoresProvider = ({children}) => {
    ...
    const [trabajadorUpdate, setTrabajadorUpdate] =useState(
        trabajador
    )
   ...
   const trabajadorCreateItem = async ()=>{
     ...
     const trabajadorUpdateItem = async () => {
        try {
            const response = await fetch(`${URL}/trabajadores/${trabajador.id}/update/`,{
                method:'PUT',
                headers:{
                    'Content-Type':'application/json',
                    'Authorization':`Bearer ${token}`,
   ...
   const trabajadorDeleteItem = async () => {
    try {
        let response = await fetch(`${URL}/trabajadores/${trabajador.id}/delete/`,{
          method:'DELETE',
          headers:{
            'Content-Type':'application/json',
            'Authorization':`Bearer ${token}`
...
const trabajadoresFiltrados = () => {
    ...
    const filtered = trabajadores.filter(trabajador => trabajador.trabajador.toLowerCase().includes(query))
    return filtered
}
